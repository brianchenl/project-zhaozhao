...
博物馆、纪念馆、档案馆等机构官网可以入 `sources`。`publisher` 写机构全称，`url` 写具体页面而非首页，`locator` 写栏目、页面标题或章节锚点，`accessed` 写实际查阅日期。

网页会被修改或下线，引用时应同时保存快照，并在该项加 `archivedUrl` 记录存档链接。同一机构官网的不同页面或子页，不因页面数量多而视为多个独立来源。

官网陈述须区分「史实陈述」与「纪念性或宣传性表述」；遇难人数、罪行定性等争议口径仍按高风险条目走人工核对与多源交叉验证，不以单个官网为最终依据。
